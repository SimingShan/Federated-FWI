    ...
    for client_id, client_metrics in metrics:
        for key, value in client_metrics.items():
            if key == "detailed_data":
                ...
                # Use the client_id from the deserialized data, not from Flower's tuple
                actual_client_id = deserialized.get("client_id", client_id)
                detailed_data_store.append({"client_id": actual_client_id, "data": deserialized})
                continue
